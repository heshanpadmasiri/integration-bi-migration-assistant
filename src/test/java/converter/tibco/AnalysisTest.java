...
public class AnalysisTest {
    ...
    public void test() throws IOException, ParserConfigurationException, SAXException {
        ...
        assertTrue(source instanceof TibcoModel.Scope.Flow.Activity.Reply reply &&
                reply.name().equals("postOut"));
        ...
        assertTrue(destinations.stream().allMatch(each -> each instanceof TibcoModel.Scope.Flow.Activity.ExtActivity));
        assertTrue(destinations.stream().flatMap(each -> analysisData.sources(each).stream())
                .allMatch(link -> link.equals(FICOScoreTopostOut)));

        destinations = analysisData.destinations(ExperianScoreTopostOut);
        assertEquals(destinations.size(), 1);
        assertTrue(destinations.stream().allMatch(each -> each instanceof TibcoModel.Scope.Flow.Activity.ExtActivity));
        assertTrue(destinations.stream().flatMap(each -> analysisData.sources(each).stream())
                .allMatch(link -> link.equals(ExperianScoreTopostOut)));
    }
}
